<template>
	<el-upload class="avatar-uploader" :headers="header" action="http://localhost:3003/api/upload/goods" :data="{type:type}" :show-file-list="false"
		...
		methods: {
			...
			handleAvatarSuccess(res, file, fileList) {
				if (res.status) {
					...
					this.$emit('update:url',res.mdImg)
				}
			},
